# -*- coding: utf-8 -*-
"""
Created on Wed Oct 19 06:18:12 2022
...
def dados_saida(x, y):
    ...
    return x_log, y_log, xordens, yordens


def det_dimensao(x_log, y_log, order=1):
    a, b = np.polyfit(x_log, y_log, order)
    divider_D = 1-a
    return divider_D


def main_graficos(x, y, x_log, y_log, xordens, y_ordens,
                  nordens, weierstrass_D):
...
def main():
    ...
    x_log, y_log, xordens, yordens = dados_saida(x, y)
    divider_D = det_dimensao(x_log, y_log)
    return divider_D
